...
bool isNewTrack(vector<struct GeneralAxis>& tmpTree,
	int currentindex,
	struct TrackDetail* nexttrack) {
	int i = currentindex;
	while (i >= 0) {
		if (tmpTree[i].istrack) {
			if (nexttrack->channel_x == tmpTree[i].track->channel_x && nexttrack->channel_y == tmpTree[i].track->channel_y && nexttrack->xaxis == tmpTree[i].track->xaxis) {
				return false;
			}
		}
		i = tmpTree[i].tmpTreefather;
	}
	return true;
}

void pushtracktobumpSync(vector<struct GeneralAxis>& tmpList,
...
void pushtracktotrackSync(vector<struct GeneralAxis>& tmpList,
	const struct RoutingParameter setting,
	struct TrackDetail& currenttrack,
	array<vector<vector<vector<struct TrackDetail>>>, 2> & trackdetail,
	vector<struct GeneralAxis>& tmpTree,
	...
	if (tmpTree[tmpTreefatherindex].isbump && tmpTree[tmpTreefatherindex].bump->dir == 0) {
		...
		if (currenttrack.xaxis) {
			...
			if (x < setting.channel_grid_width - 1) {
				...
				if (x + 1 < setting.channel_grid_width) {
					...
					if (!tmpnode.track->intree && isNewTrack(tmpTree,currentindex,tmpnode.track)) {
...
void pushbumptotrackSync(vector<struct GeneralAxis>& tmpList,
	const struct RoutingParameter setting,
		...
		if (toblist[dir][x][y].hori[horizon] == -1) {
			...
			for (vertical = vertgroup * setting.bump_grid_width; vertical < (vertgroup + 1) * setting.bump_grid_width; vertical++) {
				if (toblist[dir][x][y].vert[vertical] == -1) {
					...
					if (toblist[dir][x][y].trac[outtrack] == -1 && !candidatetrack.intree && isNewTrack(tmpTree, currentindex, &candidatetrack)) {
					...
					if (toblist[dir][x][y].trac[outtrack] == -1 && !candidatetrack2.intree && isNewTrack(tmpTree, currentindex, &candidatetrack)) {
...
vector<struct GeneralAxis> findSyncNet(const struct RoutingParameter setting,
	...
	if (currentPoint.isbump) {
		...
		//push top tracks
		pushbumptotrackSync(tmpList, setting, currentbump, trackdetail, toblist, tmpTree, currentindex);
	}
	else if (currentPoint.istrack) {
		struct TrackDetail& currenttrack = *currentPoint.track;
		pushtracktotrackSync(tmpList, setting, currenttrack, trackdetail, tmpTree, currentindex);
		pushtracktobumpSync(tmpList, setting, currenttrack, trackdetail, toblist, net, netcheck, maxStep, tmpTree, currentindex);
	}
